...
get_app_dir <- function() {
  ...
  for (cand in candidates) {
    ...
    data_here <- file.path(cand, "RawData")
    if (file.exists(helper_here) && dir.exists(data_here)) {
      return(cand)
    }

    parent <- dirname(cand)
    helper_parent <- file.path(parent, "INOR_query_tool.R")
    data_parent <- file.path(parent, "RawData")
    if (file.exists(helper_parent) && dir.exists(data_parent)) {
      return(parent)
    }
  }
  ...
  stop(
    paste0(
      "Could not find the full app folder. ",
      "Download and unzip the complete bundle so that app.R, INOR_query_tool.R, and RawData are together."
    ),
    call. = FALSE
  )
}
...
no_shared_links <- all(linkage_table$Shared_FORM_RESPONSE_GROUP_ID[1:7] == 0)
...
ui <- navbarPage(
  ...
  tabPanel(
    ...
    tags$ul(
      ...
      tags$li("Outcome comparison by consultant, implant fixation type, knee system, femoral component, and acetabular component."),
