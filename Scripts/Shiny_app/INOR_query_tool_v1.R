...
resolve_data_file <- function(filename) {
  ...
  candidates <- unique(c(
    file.path(base_dir, "RawData", filename),
    file.path(base_dir, filename),
    file.path(dirname(base_dir), "RawData", filename),
    file.path(dirname(base_dir), filename),
    file.path(cwd, "RawData", filename),
  ...
  if (!length(matches)) {
    stop(
      paste0(
        "Could not locate required data file: ", filename, ". Looked in: ",
...
# Helper: find file matching a pattern, ignoring the date/time stamp
find_data_file <- function(prefix, suffix = ".csv") {
  pattern <- paste0("^", prefix, "_\\d{4}-\\d{2}-\\d{2}_\\d{2}-\\d{2}-\\d{2}", suffix, "$")
  files <- list.files(data_dir, pattern = pattern, full.names = FALSE)
  if (length(files) == 0) stop("No file found matching: ", prefix, " ... ", suffix)
  if (length(files) > 1) warning("Multiple matches for ", prefix, "; using most recent.")
  resolve_data_file(sort(files, decreasing = TRUE)[1])
}

path_preop        <- find_data_file("NAP_INOR_PREOP_ASSESSMENT_V1")
path_postop_v1    <- find_data_file("NAP_INOR_POST_OP_ASSESSMENT_V1")
path_postop_v2    <- find_data_file("NAP_INOR_POST_OP_ASSESSMENT_V2")
path_oks          <- find_data_file("NAP_INOR_OXFORD_KNEE_SCORE")
path_ohs          <- find_data_file("NAP_INOR_OXFORD_HIP_SCORE")
path_eq5d         <- find_data_file("NAP_INOR_EQ5D")
path_comp_v1_data <- find_data_file("NAP_INOR_COMPONENT_LOG_V1", "_Data\\.csv")
path_comp_v1_comp <- find_data_file("NAP_INOR_COMPONENT_LOG_V1", "_ComponentData\\.csv")
path_comp_v2_data <- find_data_file("NAP_INOR_COMPONENT_LOG_V2", "_Data\\.csv")
path_comp_v2_comp <- find_data_file("NAP_INOR_COMPONENT_LOG_V2", "_ComponentData\\.csv")
