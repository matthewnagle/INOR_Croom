...
# =============================================================================
# SECTION 9: V7 OPERATIVE REGISTRY AND PERI-OP COMPLICATION OVERRIDES
# =============================================================================
# These overrides make the case registry peri-op aware, so operative-case counts
# come from the peri-op sheets when they are present. Pre-op remains available as
# supplemental metadata (BMI, co-morbidities, procedure code, etc.).

inor_query_tool_version <- "v7"

coalesce_nonempty_numeric <- function(primary, fallback) {
  primary_num <- suppressWarnings(as.numeric(primary))
  fallback_num <- suppressWarnings(as.numeric(fallback))
  dplyr::coalesce(primary_num, fallback_num)
}

safe_char <- function(df, candidates) {
  col_name <- resolve_optional_column_name(df, candidates)
  if (is.null(col_name)) {
    return(rep(NA_character_, nrow(df)))
  }
  blank_to_na(df[[col_name]])
}

safe_date_char <- function(df, candidates) {
  col_name <- resolve_optional_column_name(df, candidates)
  if (is.null(col_name)) {
    return(rep(NA_character_, nrow(df)))
  }
...
safe_date_value <- function(df, candidates) {
  parse_date_flexible(safe_date_char(df, candidates))
}

first_non_missing_num_v7 <- function(x) {
...
build_preop_case_rows_v7 <- function(df) {
  if (is.null(df) || !nrow(df)) {
    return(tibble::tibble())
  }

  tibble::tibble(
    FORM_RESPONSE_GROUP_ID = safe_char(df, "FORM_RESPONSE_GROUP_ID"),
    `Patient Id` = safe_char(df, "Patient Id"),
    `MRN Number` = safe_char(df, "MRN Number"),
    `First Name` = safe_char(df, "First Name"),
    `Last Name` = safe_char(df, "Last Name"),
    Sex = safe_char(df, "Sex"),
    `Date of Birth` = safe_char(df, "Date of Birth"),
    `Patient age at time of surgery` = safe_char(df, c("Patient age at time of surgery", "Patient Age")),
    Joint = safe_char(df, "Joint"),
    Laterality = safe_char(df, "Laterality"),
    `Procedure type` = safe_char(df, "Procedure type"),
    `Procedure code` = safe_char(df, "Procedure code"),
    `Procedure Description` = safe_char(df, c("Procedure Description", "Procedure description")),
    `Procedure date` = safe_date_char(df, c("Procedure date", "Date of surgery")),
    BMI = suppressWarnings(as.numeric(safe_char(df, "BMI"))),
    `Co-morbidities` = safe_char(df, "Co-morbidities"),
    `Admitting consultant` = safe_char(df, "Admitting consultant"),
    `Surgeon grade` = safe_char(df, c("Surgeon grade", "Lead Operating surgeon grade", "Operating surgeon grade")),
    `Operating surgeon` = safe_char(df, c("Operating surgeon", "Lead Operating surgeon name")),
    ACCESS_POINT_NAME = safe_char(df, c("ACCESS_POINT_NAME", "HOSPITAL_NAME")),
    HOSPITAL_NAME = safe_char(df, c("HOSPITAL_NAME", "ACCESS_POINT_NAME")),
    `Case registry source` = "Pre-op"
  ) %>%
    filter(!is.na(FORM_RESPONSE_GROUP_ID) & FORM_RESPONSE_GROUP_ID != "")
}

build_periop_case_rows_v7 <- function(df, source_label) {
  if (is.null(df) || !nrow(df)) {
    return(tibble::tibble())
  }
  ...
  tibble::tibble(
    ...
    Laterality = safe_char(df, "Laterality"),
    `Procedure type` = safe_char(df, "Procedure type"),
    `Procedure code` = safe_char(df, c("Procedure code")),
    `Procedure Description` = safe_char(df, c("Procedure Description", "Procedure description")),
    `Procedure date` = safe_date_char(df, c("Date of surgery", "Procedure date")),
    BMI = suppressWarnings(as.numeric(rep(NA_character_, nrow(df)))),
    `Co-morbidities` = safe_char(df, c("Co-morbidities")),
    `Admitting consultant` = safe_char(df, "Admitting consultant"),
    `Surgeon grade` = safe_char(df, c("Lead Operating surgeon grade", "Operating surgeon grade", "Surgeon grade")),
    `Operating surgeon` = safe_char(df, c("Lead Operating surgeon name", "Operating surgeon")),
    ACCESS_POINT_NAME = safe_char(df, c("ACCESS_POINT_NAME", "HOSPITAL_NAME")),
    HOSPITAL_NAME = safe_char(df, c("HOSPITAL_NAME", "ACCESS_POINT_NAME")),
    `Case registry source` = source_label
  ) %>%
    filter(!is.na(FORM_RESPONSE_GROUP_ID) & FORM_RESPONSE_GROUP_ID != "")
}

collapse_case_rows_v7 <- function(df) {
  if (is.null(df) || !nrow(df)) {
    return(tibble::tibble())
  }
  ...
  df %>%
    group_by(FORM_RESPONSE_GROUP_ID) %>%
    summarise(
      `Patient Id` = first_non_missing_chr(`Patient Id`),
      `MRN Number` = first_non_missing_chr(`MRN Number`),
      `First Name` = first_non_missing_chr(`First Name`),
      `Last Name` = first_non_missing_chr(`Last Name`),
      Sex = first_non_missing_chr(Sex),
      `Date of Birth` = first_non_missing_chr(`Date of Birth`),
      `Patient age at time of surgery` = first_non_missing_chr(`Patient age at time of surgery`),
      Joint = first_non_missing_chr(Joint),
      Laterality = first_non_missing_chr(Laterality),
      `Procedure type` = first_non_missing_chr(`Procedure type`),
      `Procedure code` = first_non_missing_chr(`Procedure code`),
      `Procedure Description` = first_non_missing_chr(`Procedure Description`),
      `Procedure date` = as.character(first_non_missing_date(parse_date_flexible(`Procedure date`))),
      BMI = first_non_missing_num_v7(BMI),
      `Co-morbidities` = first_non_missing_chr(`Co-morbidities`),
      `Admitting consultant` = first_non_missing_chr(`Admitting consultant`),
      `Surgeon grade` = first_non_missing_chr(`Surgeon grade`),
      `Operating surgeon` = first_non_missing_chr(`Operating surgeon`),
      ACCESS_POINT_NAME = first_non_missing_chr(ACCESS_POINT_NAME),
      HOSPITAL_NAME = first_non_missing_chr(HOSPITAL_NAME),
      `Case registry source` = first_non_missing_chr(`Case registry source`),
      .groups = "drop"
    )
}

preop_case_registry <- collapse_case_rows_v7(build_preop_case_rows_v7(preop))
periop_case_registry <- collapse_case_rows_v7(bind_rows(
  build_periop_case_rows_v7(periop_v2, "Peri-op V2"),
  build_periop_case_rows_v7(periop_v1, "Peri-op V1")
))

if (nrow(periop_case_registry) > 0 && nrow(preop_case_registry) > 0) {
  periop_case_registry <- periop_case_registry %>%
    left_join(
      preop_case_registry %>%
        select(
          FORM_RESPONSE_GROUP_ID,
          preop_dob = `Date of Birth`,
          preop_bmi = BMI,
          preop_comorbidities = `Co-morbidities`,
          preop_procedure_code = `Procedure code`,
          preop_consultant = `Admitting consultant`,
          preop_surgeon_grade = `Surgeon grade`,
          preop_operating_surgeon = `Operating surgeon`,
          preop_patient_age = `Patient age at time of surgery`
        ),
      by = "FORM_RESPONSE_GROUP_ID"
    ) %>%
    mutate(
      `Date of Birth` = coalesce_nonempty_character(`Date of Birth`, preop_dob),
      BMI = dplyr::coalesce(BMI, preop_bmi),
      `Co-morbidities` = coalesce_nonempty_character(`Co-morbidities`, preop_comorbidities),
      `Procedure code` = coalesce_nonempty_character(`Procedure code`, preop_procedure_code),
      `Admitting consultant` = coalesce_nonempty_character(`Admitting consultant`, preop_consultant),
      `Surgeon grade` = coalesce_nonempty_character(`Surgeon grade`, preop_surgeon_grade),
      `Operating surgeon` = coalesce_nonempty_character(`Operating surgeon`, preop_operating_surgeon),
      `Patient age at time of surgery` = coalesce_nonempty_character(`Patient age at time of surgery`, preop_patient_age)
    ) %>%
    select(-starts_with("preop_"))
}

case_metadata_registry <- collapse_case_rows_v7(bind_rows(
  periop_case_registry,
  preop_case_registry
))

operative_case_registry <- if (nrow(periop_case_registry) > 0) {
  periop_case_registry
} else {
  preop_case_registry
}

cases <- operative_case_registry %>%
  transmute(
    case_id = FORM_RESPONSE_GROUP_ID,
    patient_id = `Patient Id`,
    mrn = `MRN Number`,
    ...
    operating_surgeon = `Operating surgeon`,
    hospital = ACCESS_POINT_NAME,
    case_registry_source = `Case registry source`
  )
...
summarise_demographics_by_consultant <- function(consultant = NULL, joint = NULL) {
  ...
  comorb_cols <- names(preop)[grepl("^Co-morbidities region:", names(preop))]
  comorb_lookup <- if (length(comorb_cols) > 0) {
    preop %>% select(FORM_RESPONSE_GROUP_ID, all_of(comorb_cols))
  } else {
    NULL
  }
  if (!is.null(comorb_lookup)) {
    df <- df %>% left_join(comorb_lookup, by = "FORM_RESPONSE_GROUP_ID")
  }
  ...
  if (length(comorb_cols) > 0) {
    comorb_detail <- df %>%
      ...
      mutate(comorbidity_type = gsub("Co-morbidities region:", "", comorbidity_type)) %>%
      group_by(`Admitting consultant`, comorbidity_type) %>%
      summarise(n_yes = sum(present == "Y", na.rm = TRUE), .groups = "drop") %>%
      left_join(df %>% count(`Admitting consultant`, name = "total_cases"), by = "Admitting consultant") %>%
      mutate(pct = round(n_yes / total_cases * 100, 1)) %>%
      arrange(`Admitting consultant`, desc(n_yes))
  } else {
    comorb_detail <- tibble::tibble()
  }

  list(summary = demo_summary, comorbidities = comorb_detail)
}
...
summarise_demographics <- function(joint = NULL) {
  ...
  tibble::tibble(
    ...
    bmi_mean = round(mean(df$BMI, na.rm = TRUE), 1),
    bmi_sd = round(sd(df$BMI, na.rm = TRUE), 1),
    bmi_range = paste0(round(min(df$BMI, na.rm = TRUE), 1), "-", round(max(df$BMI, na.rm = TRUE), 1)),
    pct_comorbid = round(sum(df$`Co-morbidities` == "Yes", na.rm = TRUE) / nrow(df) * 100, 1),
    n_hip = sum(df$Joint == "Hip", na.rm = TRUE),
    n_knee = sum(df$Joint == "Knee", na.rm = TRUE)
  )
...
parse_periop_comp_column_v7 <- function(col_name) {
  clean_name <- gsub("\\s+", " ", trimws(col_name))
  side_match <- regmatches(clean_name, regexpr("\\((Left|Right)\\)", clean_name, perl = TRUE))
  side <- if (length(side_match) && nzchar(side_match)) gsub("[()]", "", side_match) else NA_character_

  prefix <- sub("\\s*\\((Left|Right)\\).*", "", clean_name, perl = TRUE)
  prefix <- gsub("^Hip - ", "", prefix)
  prefix <- gsub("^Knee - ", "", prefix)
  prefix <- trimws(prefix)

  family <- if (grepl("Ligament", prefix, ignore.case = TRUE)) {
    "Ligament complication"
  } else if (grepl("Fracture", prefix, ignore.case = TRUE)) {
    "Fracture"
  } else if (grepl("Complication", prefix, ignore.case = TRUE)) {
    "Complication"
  } else {
    prefix
  }

  label <- if (grepl(":", clean_name, fixed = TRUE)) {
    trimws(sub(".*:", "", clean_name))
  } else if (family == "Fracture") {
    "Fracture (unspecified)"
  } else if (family == "Ligament complication") {
    "Ligament complication (unspecified)"
  } else if (family == "Complication") {
    "Complication (unspecified)"
  } else {
    prefix
  }

  list(side = side, family = family, label = label)
}

build_periop_complications_from_dataset_v7 <- function(df, source_label) {
  if (is.null(df) || !nrow(df)) {
    return(tibble::tibble())
  }
  ...
  base_df <- tibble::tibble(
    ...
    `Admitting consultant` = safe_char(df, "Admitting consultant"),
    `Surgeon grade` = safe_char(df, c("Lead Operating surgeon grade", "Operating surgeon grade", "Surgeon grade")),
    hospital = safe_char(df, c("ACCESS_POINT_NAME", "HOSPITAL_NAME")),
    assessment_date = safe_date_char(df, c("Date of surgery", "Procedure date")),
    source = source_label
  )

  all_cols <- names(df)
  specific_cols <- all_cols[
    !grepl("^Indication", all_cols, ignore.case = TRUE) &
      grepl("type", all_cols, ignore.case = TRUE) &
      grepl("(Complication|Complications|Fracture|Ligament)", all_cols, ignore.case = TRUE) &
      grepl("\\((Left|Right)\\)", all_cols, perl = TRUE)
  ]

  parent_cols <- all_cols[
    !grepl("^Indication", all_cols, ignore.case = TRUE) &
      grepl("(Complication|Complications|Fracture|Ligament)", all_cols, ignore.case = TRUE) &
      !grepl("type", all_cols, ignore.case = TRUE) &
      grepl("\\((Left|Right)\\)$", all_cols, perl = TRUE)
  ...
  build_long <- function(columns, is_parent = FALSE) {
    if (!length(columns)) return(tibble::tibble())

    parts <- lapply(columns, function(col_name) {
      yes_idx <- which(toupper(blank_to_na(df[[col_name]])) %in% c("Y", "YES", "TRUE", "1"))
      if (!length(yes_idx)) return(NULL)
      meta <- parse_periop_comp_column_v7(col_name)
      tibble::tibble(
        .row_id = yes_idx,
        complication_column = col_name,
        side = meta$side,
        complication_group = meta$family,
        complication = meta$label,
        detail = if (is_parent) paste0(meta$family, " flag") else meta$label,
        is_parent = is_parent
      )
    })
  ...
  specific_long <- build_long(specific_cols, is_parent = FALSE)
  parent_long <- build_long(parent_cols, is_parent = TRUE)

  if (nrow(parent_long) > 0) {
    if (nrow(specific_long) > 0) {
      parent_long <- parent_long %>%
        anti_join(
          specific_long %>% distinct(.row_id, side, complication_group),
          by = c(".row_id", "side", "complication_group")
        )
    }
  }

  bind_rows(specific_long, parent_long) %>%
    left_join(base_df, by = ".row_id") %>%
    transmute(
      FORM_RESPONSE_GROUP_ID,
      `Patient Id`,
      `MRN Number`,
      ...
      source,
      phase = "Peri-op",
      presentation = "Peri-op assessment",
      side,
      complication_group,
      complication,
      detail
...
periop_complications_long <- bind_rows(
  build_periop_complications_from_dataset_v7(periop_v2, "Peri-op V2"),
  build_periop_complications_from_dataset_v7(periop_v1, "Peri-op V1")
)

get_periop_complications <- function(case_id = NULL, mrn = NULL, patient_id = NULL,
                                     complication_type = NULL, source = NULL) {
  result <- periop_complications_long
  if (!is.null(case_id)) result <- result %>% filter(FORM_RESPONSE_GROUP_ID == case_id)
  if (!is.null(mrn)) result <- result %>% filter(`MRN Number` == mrn)
  if (!is.null(patient_id)) result <- result %>% filter(`Patient Id` == patient_id)
  if (!is.null(complication_type)) result <- result %>% filter(complication == complication_type)
  if (!is.null(source)) result <- result %>% filter(source == source)
  result
}

cat(sprintf(
  "V7 operative registry ready: %d operative cases (peri-op preferred), %d metadata cases, %d peri-op complication events.\n",
  nrow(operative_case_registry),
  nrow(case_metadata_registry),
  nrow(periop_complications_long)
))
